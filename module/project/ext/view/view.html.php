      ...
      <div class="row">
        ...
        <div class="col-sm-6">
          <div class="panel block-docs" style="height: 240px">
            <div class="panel-heading">
            <div class="panel-title"><?php echo $lang->project->doclib;?> (<?php echo $filecount; ?>)</div>
              <nav class="panel-actions nav nav-default">
                <li><?php common::printLink('doc', 'objectLibs', "type=project&projectID=$project->id&from=project", '<i class="icon icon-more icon-sm"></i>', '', "title=$lang->more");?>
                 </li>
              </nav>
            </div>
            <div class="panel-body">
              <div class="row row-grid">
                ...
                <?php $i = 0; 
                echo $this->fetch('file', 'printFiles', array('files' => $files, 'fieldset' => 'false'));
                ?>
                    <form class='form-indicator main-form form-ajax clearfix' method='post' id='dataform'>
                      <?php echo $this->fetch('file', 'buildForm'); ?>
                      <input type="submit" value="Upload Files" class="btn btn-sm pull-right" />
                    </form>
                <?php foreach($docLibs as $libID => $docLib):?>
                <?php if($i > (8 - $filecount)) break;?>
                <div class="col-xs-6">
                  <?php if($libID == 'files'):?>
          ...
          <div class="cell">
            <div class="detail">
              ...
              <div class="detail-content article-content">
                <p><span class="text-limit" data-limit-size="40"><?php echo nl2br(html_entity_decode($project->desc));?></span><a class="text-primary text-limit-toggle small" data-text-expand="<?php echo $lang->expand;?>"  data-text-collapse="<?php echo $lang->collapse;?>"></a></p>
                <p>
            ...
            <?php if(strpos($project->desc, '[Product Dev]') !== false): ?>
            <div class="detail">
              <!-- <div class="detail-title"><strong><?php echo $lang->project->basicInfo;?></strong></div> -->
              <div class="detail-content">
                  <!-- BUTTON HERE
                  <?php common::printLink('project', 'briefdesign', "projectID=$project->id", "" . $lang->project->briefdesign, '', "class='btn btn-primary iframe'", true, true); ?>
                   -->
...
<div id="mainActions">
  ...
  <div class="btn-toolbar">
    ...
    if(!$project->deleted)
    {
        ...
        common::printIcon('project', 'closeall', "projectID=$project->id", $project, 'button', '', '', 'iframe', true);
        common::printIcon('project', 'done',     "projectID=$project->id", $project, 'button', '', '', 'iframe', true);

        echo "<div class='divider'></div>";
        common::printIcon('project', 'duplicate', $params, $project, 'button', '', '', 'iframe', true);
